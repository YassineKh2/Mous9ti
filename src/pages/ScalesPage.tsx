...
export const ScalesPage: React.FC = () => {
  ...
  const [selectedScale, setSelectedScale] = useState<ScaleDefinition>(SCALES_DATABASE[4]); // A Minor Pentatonic
  ...
  const [pianoFocusRange, setPianoFocusRange] = useState<'all' | 'octave4' | 'octave3' | 'octave34'>('octave4');
  const [instrumentView, setInstrumentView] = useState<'guitar' | 'piano' | 'both'>('guitar');

  // Scale Playback Direction & Animation State
  const [playDirection, setPlayDirection] = useState<PlaybackDirection>('ascending');
  const [isPlaying, setIsPlaying] = useState<boolean>(false);
  const [activePlayingNote, setActivePlayingNote] = useState<string | null>(null);
  const [activePlayingOctave, setActivePlayingOctave] = useState<number | null>(null);
  const [activePlayingString, setActivePlayingString] = useState<number | null>(null);
  const [activePlayingFret, setActivePlayingFret] = useState<number | null>(null);
  ...
  const scaleCategories = ['Major & Minor', 'Pentatonic & Blues', 'Modes', 'Symmetrical & Exotic'] as const;
  ...
  const handlePlayScale = (direction: PlaybackDirection = playDirection) => {
    ...
    const scaleSemitones = new Set(selectedScale.intervals.map((iv) => (rootSemitone + iv) % 12));
    ...
    if (activeCagedBox && selectedScale.cagedBoxes && selectedScale.cagedBoxes[activeCagedBox]) {
      ...
      const shapeNotes: { note: NoteName; octave: number; stringIdx: number; fretNum: number }[] = [];

      // Traverse from lowest string (6th string, index 0) to highest string (1st string, index 5)
      for (let stringIdx = 0; stringIdx < currentTuning.strings.length; stringIdx++) {
        ...
        for (let fret = Math.max(0, primaryStart); fret <= Math.min(fretCount, primaryEnd); fret++) {
          const currentSemitone = (openSemitone + fret) % 12;
          if (scaleSemitones.has(currentSemitone)) {
            const noteName = getSpelledNote(currentSemitone, { root: selectedRoot, scale: selectedScale });
            const totalSemitonesFromOpen = openSemitone + fret;
            const octave = stringBaseOctave + Math.floor(totalSemitonesFromOpen / 12) - Math.floor(openSemitone / 12);
            shapeNotes.push({
              note: noteName,
              octave,
              stringIdx,
    ...
    if (playSequence.length === 0 && (instrumentView === 'piano' || instrumentView === 'both') && pianoFocusRange !== 'all') {
      ...
      for (let o = 0; o < numOctaves; o++) {
        ...
        selectedScale.intervals.forEach((interval) => {
          ...
          const noteName = getSpelledNote(semitonesTotal % 12, { root: selectedRoot, scale: selectedScale });
  ...
  const filteredModalScales = SCALES_DATABASE.filter((s) => {
    const matchesCategory = scaleMenuCategory === 'All' || s.category === scaleMenuCategory;
    const matchesSearch = 
      ...
      <div className="bg-surface-container border border-outline-variant/30 rounded-lg p-6 shadow-xl space-y-4">
        <div className="flex flex-wrap items-center justify-between gap-4 pb-3 border-b border-outline-variant/30">
              ...
              >
                <div className="flex items-center gap-1.5">
                  <h2 className="font-mono text-lg font-bold text-on-surface group-hover:text-primary transition-colors flex items-center gap-1.5">
                    <span>{selectedRoot} {selectedScale.name}</span>
                    <ChevronDown size={18} className="text-on-surface-variant group-hover:text-primary transition-transform duration-200 group-hover:translate-y-0.5" />
                  </h2>
                </div>
                <div className="flex items-center gap-2 mt-0.5">
                  <span className="text-xs font-mono text-on-surface-variant">
                    Formula: <strong className="text-on-surface">{selectedScale.formula}</strong>
                  </span>
          ...
          <div className="flex items-center gap-3 flex-wrap">
            ...
            <div className="relative inline-flex items-center" ref={playMenuRef}>
              {isPlaying ? (
                <button
                  type="button"
                ...
                <div className="inline-flex rounded-lg shadow-md bg-primary text-on-primary">
                  ...
                  >
                    <Volume2 size={15} />
                    <span>
                      PLAY {playDirection === 'ascending' ? 'ASCENDING' : playDirection === 'descending' ? 'DESCENDING' : 'ASC & DESC'}
                    </span>
                  </button>
                  ...
                  >
                    <ChevronDown size={14} className={`transition-transform duration-200 ${isPlayMenuOpen ? 'rotate-180' : ''}`} />
                  </button>
                </div>
              )}
              ...
              {isPlayMenuOpen && (
                <div className="absolute right-0 top-full mt-2 z-50 bg-surface border border-outline-variant/40 rounded-xl p-2 shadow-2xl w-60 animate-in fade-in zoom-in-95 duration-150">
                  ...
                  <div className="space-y-1">
                    {[
                      { id: 'ascending' as PlaybackDirection, label: 'Ascending', sub: 'Low to High (1 → 8)', icon: ArrowUpRight },
                      { id: 'descending' as PlaybackDirection, label: 'Descending', sub: 'High to Low (8 → 1)', icon: ArrowDownRight },
                      { id: 'both' as PlaybackDirection, label: 'Asc & Desc', sub: 'Up & down continuous', icon: ArrowUpDown },
                    ].map((opt) => {
                      ...
                      return (
                        ...
                        >
                          <div className="flex items-center gap-2">
                            <Icon size={14} className={isSelected ? 'text-secondary' : 'text-on-surface-variant'} />
                            <div>
                              <div className="font-semibold leading-tight">{opt.label}</div>
                              <div className="text-[10px] opacity-70 font-normal">{opt.sub}</div>
                            </div>
                          </div>
                          {isSelected && <Check size={14} className="text-secondary shrink-0 ml-1" />}
                        </button>
                      );
                    })}
                  </div>
        ...
        <div className="grid grid-cols-2 sm:grid-cols-4 md:grid-cols-7 gap-3">
          {scaleNotes.map((sn, idx) => {
            ...
            return (
              ...
              >
                <span className={`text-[10px] font-mono uppercase ${isCurrentlyPlaying ? 'text-on-secondary/80' : 'text-on-surface-variant'}`}>
                  {isRoot ? 'Root (1)' : `Degree ${sn.degree}`}
                </span>
                <span className={`font-mono text-xl font-bold ${isCurrentlyPlaying ? 'text-on-secondary font-black' : isRoot ? 'text-secondary' : 'text-on-surface'}`}>
                  {sn.note}
                </span>
                <span className={`text-[10px] font-mono px-2 py-0.5 rounded ${isCurrentlyPlaying ? 'bg-black/20 text-on-secondary font-semibold' : 'bg-surface border border-outline-variant/40 text-on-surface-variant'}`}>
        ...
        {(instrumentView === 'guitar' || instrumentView === 'both') && selectedScale.cagedBoxes && (
          ...
          <div className="pt-3 border-t border-outline-variant/30 space-y-2">
            <div className="flex items-center justify-between">
              ...
              <span className="text-[11px] font-mono text-on-surface-variant hidden sm:inline">
                Dims notes outside active range (e.g. before {selectedRoot}4 & after {selectedRoot}5)
              </span>
            </div>
