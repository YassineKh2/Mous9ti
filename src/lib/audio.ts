...
class AudioEngine {
  ...
  private subdivision: 'quarter' | 'eighth' | 'sixteenth' | 'triplet' = 'quarter';
  private soundType: 'click' | 'woodblock' | 'tick' | 'beep' = 'click';
  private volume: number = 0.8;
  private onBeatCallback: ((beat: number, isAccent: boolean, isSubdivision: boolean) => void) | null = null;
  ...
  public playGuitarPluck(note: string, octave: number = 3, duration: number = 2.0, timeOffset: number = 0) {
    ...
    filter.frequency.exponentialRampToValueAtTime(Math.max(freq * 1.5, 200), startTime + duration);
    ...
    masterPluckGain.gain.exponentialRampToValueAtTime(0.0001, startTime + duration); // Ringing tail
  ...
  public playPianoNote(note: string, octave: number = 4, duration: number = 2.5, timeOffset: number = 0) {
    if (this.isMuted) return;
    const ctx = this.getContext();
    const startTime = ctx.currentTime + timeOffset;
    const freq = this.noteToFrequency(note, octave);
    ...
    filter.frequency.exponentialRampToValueAtTime(Math.max(freq * 2, 350), startTime + duration);
  ...
  public playChordArpeggio(notes: { note: string; octave: number }[], instrument: 'guitar' | 'piano' = 'guitar', staggerSec: number = 0.06) {
    notes.forEach((n, idx) => {
  ...
  private scheduleMetronomeTick(time: number, isAccent: boolean, isSubdivision: boolean) {
    if (this.isMuted) return;
    const ctx = this.getContext();
    ...
    if (this.soundType === 'woodblock') {
      ...
      filter.frequency.setValueAtTime(isAccent ? 1200 : isSubdivision ? 700 : 900, time);
      filter.Q.setValueAtTime(8, time);

      osc.type = 'sine';
      osc.frequency.setValueAtTime(isAccent ? 960 : isSubdivision ? 600 : 780, time);
      ...
      const filter = ctx.createBiquadFilter();
      filter.type = 'bandpass';
      filter.frequency.setValueAtTime(isAccent ? 3500 : isSubdivision ? 1800 : 2600, time);
      ...
      osc.type = 'sine';
      osc.frequency.setValueAtTime(isAccent ? 1760 : isSubdivision ? 880 : 1320, time);
      ...
      osc.type = 'triangle';
      osc.frequency.setValueAtTime(isAccent ? 2400 : isSubdivision ? 1000 : 1600, time);
      osc.frequency.exponentialRampToValueAtTime(isAccent ? 300 : 200, time + 0.012);
  ...
  private scheduler() {
    ...
    while (this.nextNoteTime < ctx.currentTime + lookaheadSec) {
      ...
      if (this.onBeatCallback) {
        ...
        const timeDiff = Math.max(0, (this.nextNoteTime - ctx.currentTime) * 1000);
        setTimeout(() => {
    ...
    this.metronomeTimerId = window.setTimeout(() => this.scheduler(), scheduleIntervalMs);
  }
